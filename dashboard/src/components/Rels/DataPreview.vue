<template>
  <div>
    <h2>Relationship data preview: {{ db }}</h2>
    <hr />
    <h3>Query</h3>
    <b-row>
      <b-col>
        <p>Select meta relationship</p>
        <b-form-select
          v-if="metaRelOptions"
          v-model="metaRel"
          :options="metaRelOptions"
        />
      </b-col>
      <b-col>
        <div>
...
export default {
  ...
  props: {
    ...
    metaRelOptions: Array,
    metaRelDefault: String
  },
  data: () => ({
    metaRel: null,
    sizeLimit: 100,
  ...
  mounted: function() {
    this.metaRel = this.metaRelDefault;
  },
  computed: {
    query() {
      if (this.metaRel && this.sizeLimit) {
        return `MATCH (n)-[r:${this.metaRel}]-(m)
                RETURN labels(n), n, type(r), r, labels(m), m
                SKIP ${this.skip}
