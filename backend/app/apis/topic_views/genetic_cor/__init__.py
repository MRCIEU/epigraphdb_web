...
from .functions import GeneticCorQueryProcessor, master_name
...
ac_index_trait = "ac_genetic_cor_trait"


@router.get("/genetic-cor", response_model=bool)
def get_genetic_cor(
    trait: str,
    cor_coef_threshold: float = Query(0.8, ge=0.0, le=1.0),
    overwrite: bool = False,
) -> bool:
    """This is the master processor. For actual data use sub-apis
    """
    log_args(api="/genetic-cor", kwargs=locals())
    processor = GeneticCorQueryProcessor(
...
@router.get(
    "/genetic-cor/{endpoint}", response_model=models.standard_endpoint_response
)
def get_genetic_cor_endpoints(
    ...
    log_args(api=f"/genetic-cor/{endpoint.value}", kwargs=locals())
    processor = GeneticCorQueryProcessor(
        params={"trait": trait, "cor_coef_threshold": cor_coef_threshold}
    )
    res = None
...
@router.get("/genetic-cor/cache/drop", response_model=bool)
def get_genetic_cor_cache_drop() -> bool:
    return mongo_cache_drop(master_name=master_name)


@router.get("/genetic-cor/ac/index", response_model=bool)
def get_genetic_cor_ac_index(overwrite: bool = False) -> bool:
    log_args(api="/genetic-cor/ac/index", kwargs=locals())
...
@router.get("/genetic-cor/ac/trait", response_model=List[str])
def get_genetic_cor_ac_trait(query: str, size: int = 20) -> List[str]:
    log_args(api="/genetic-cor/ac/trait", kwargs=locals())
    if not es_client.indices.exists(index=ac_index_trait):
        get_genetic_cor_ac_index()
    res = query_ac_name(
