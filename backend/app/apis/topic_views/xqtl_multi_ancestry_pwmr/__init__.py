...
@router.get("/xqtl_multi_ancestry_pwmr/list/{entity}")
def list_ents(entity: models.Entity, overwrite: bool = False):
    log_args(api=f"/xqtl_multi_ancestry_pwmr/list/{entity}", kwargs=locals())

    def _func(entity: str):
        r = requests.get(
            f"{api_url}/xqtl_multi_ancestry_pwmr/list/{entity}",
...
@router.get("/xqtl_multi_ancestry_pwmr/xqtl_pwas_mr/{entity}")
def xqtl_pwas_mr(
    ...
    log_args(
        api=f"/xqtl_multi_ancestry_pwmr/xqtl_pwas_mr/{entity}", kwargs=locals()
    )
    params: Dict[str, Any] = {"q": q, "pval_threshold": pval_threshold}
    r = requests.get(
        f"{api_url}/xqtl_multi_ancestry_pwmr/xqtl_pwas_mr/{entity.value}",
        params=params,
        headers=api_request_headers,
    )
    r.raise_for_status()
