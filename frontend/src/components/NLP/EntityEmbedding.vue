<template lang="pug">
div
  p Return the embedding vector of a piece of an existing entity in EpiGraphDB.
  h3 Query
...
export default Vue.extend({
  ...
  computed: {
    btnState() {
      const inputNull = this.metaEnt == null || this.entId == null;
  ...
  methods: {
    async updateSearch() {
      this.embeddingRes = await requests.encodeEnt({
        metaEnt: this.metaEnt,
        entId: this.entId,
      });
      this.similarEntRes = await requests.searchSimilarEntByEnt({
        metaEnt: this.metaEnt,
        entId: this.entId,
      });
    }
