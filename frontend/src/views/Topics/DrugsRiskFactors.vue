<template>
  <div>
    <div>
      <h3>{{ pageTitle }}</h3>
      <p>Drugs for common risk factors of diseases</p>
    </div>
        ...
        <b-col>
          <div class="py-2">
            <vue-typeahead-bootstrap
              v-model="trait"
              placeholder="Enter trait, e.g. Coronary heart disease"
...
import info from "@/assets/docs/drugs-risk-factors.md";
...
export default {
  ...
  data: () => ({
    pageTitle: `Drugs for risk factors`,
    tooltipDoc: tooltips,
    ...
    alert: false,
    alertMsg: "",
    urlMaster: `${config.web_backend_url}/drugs_risk_factors`,
    infoText: info,
  }),
  mounted: function() {
    this.indexAc();
  ...
  methods: {
    setupRouteQuery() {
      ...
      if (this.$route.query["pval-threshold"]) {
        this.pval = this.$route.query["pval-threshold"];
      }
      if (this.trait) {
        this.getMaster();
      }
